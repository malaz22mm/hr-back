...
async function main() {
  ...
  for (const record of employeeRecords) {
    await prisma.employees.create({
      data: {
        ...
        health_status: record.health_status?.trim()
          ? record.health_status.trim()
          : null,
        health_state_id: parseInt(record.health_state_id),
        training_hours_last_year: parseInt(record.training_hours_last_year),
        training_hours_last_6_months: parseInt(
          record.training_hours_last_6_months,
        ),
        training_gap_score: parseInt(record.training_gap_score),
        promotion_stagnation_ratio: parseFloat(
          record.promotion_stagnation_ratio,
        ),
        ...
        environment_satisfaction_id: parseInt(
          record.environment_satisfaction_id,
        ),
        job_involvement_id: parseInt(record.job_involvement_id),
        job_satisfaction_id: parseInt(record.job_satisfaction_id),
        relationship_satisfaction_id: parseInt(
          record.relationship_satisfaction_id,
        ),
  ...
  const vacationPath = path.resolve(
    __dirname,
    './vacation_requests_updated.csv',
  );
...
async function seedLookups() {
  ...
  await prisma.workShift.createMany({
    data: [
      ...
      },
      {
        ...
        end_time: new Date('2024-01-01T22:00:00'),
        grace_period_minutes: 15,
      },
    ],
    skipDuplicates: true,
  ...
  // 0. Health State Mapping
  await prisma.healthState.createMany({
    data: [
      { id: 0, name: 'Healthy', name_code: 'healthy' },
      {
        id: 1,
        name: 'Minor Chronic Condition',
        name_code: 'minor_chronic_condition',
      },
      {
        id: 2,
        name: 'Moderate Health Issues',
        name_code: 'moderate_health_issues',
      },
      {
        id: 3,
        name: 'Serious Health Issues',
        name_code: 'serious_health_issues',
      },
    ],
    skipDuplicates: true,
  });
